...
async function openInbound(inboundId) {
  try {
    ...
    const itemRows = (po.items || []).map(it => `
      <tr><td>${it.sku_code}</td><td>${it.sku_name || '-'}</td>
      <td>${it.qty_expected}</td><td class="inb-recv" data-id="${it.id}">${it.qty_received}</td>
      <td>${it.qty_remaining > 0 ? `<span style="color:var(--warning)">${it.qty_remaining}</span>` : '<span style="color:var(--success)">✓</span>'}</td></tr>
    `).join('');
    ...
    active.innerHTML = `
      ...
      <div class="scan-container">
        ...
        <input type="text" class="scan-input" id="inbound-scan" placeholder="Scan..." autofocus>
        <div style="display:flex;gap:0.5rem;align-items:center;justify-content:center;margin-top:0.5rem">
          ...
          <input type="number" class="form-input" id="inbound-scan-qty" value="1" min="1" style="width:80px;text-align:center">
        </div>
        <div id="inbound-scan-fb" style="margin-top:0.75rem;font-size:0.9rem"></div>
      </div>
      <div class="card">
        <table style="width:100%;font-size:0.85rem"><thead><tr><th>SKU</th><th>Name</th><th>Expected</th><th>Received</th><th>Remain</th></tr></thead><tbody>${itemRows}</tbody></table>
      </div>
    ...
    scanInput.addEventListener('keydown', async (e) => {
      if (e.key === 'Enter' && scanInput.value.trim()) {
        ...
        const qty = parseInt(document.getElementById('inbound-scan-qty').value) || 1;
        scanInput.value = '';
        try {
          const r = await API.post(`/inbound/${inboundId}/scan?event_id=${window.currentEventId}`, { barcode, qty });
          const d = r.data;
          document.getElementById('inbound-scan-fb').innerHTML = `<span style="color:var(--success)">✓ ${d.sku_code} received (${d.qty_received}/${d.qty_expected})</span>`;
          const cell = document.querySelector(`.inb-recv[data-id="${d.item_id}"]`);
          if (cell) cell.textContent = d.qty_received;
        } catch(err) {
          document.getElementById('inbound-scan-fb').innerHTML = `<span style="color:var(--danger)">✗ ${err.message}</span>`;
        }
        scanInput.focus();
      }
    });
